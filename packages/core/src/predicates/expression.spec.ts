...
describe("predicates/expression", () => {
  ...
  it("ref: resolves to true only when the predicate is exactly boolean true", () => {
    const predicate = Expr.compile(Expr.ref("suitest_camera_connected"));
  ...
  it("equals: compares against a non-boolean value", () => {
    const predicate = Expr.compile(Expr.equals("suitest_status", "ready"));
  ...
  it("includes: substring match on a stringified value", () => {
    const predicate = Expr.compile(Expr.includes("suitest_message", "timeout"));
  ...
  it("and: true only when every sub-expression is true", () => {
    const predicate = Expr.compile(
      BooleanTree.and([Expr.ref("suitest_camera_recording"), Expr.ref("suitest_camera_streaming")]),
    );
  ...
  it("or + not: true when recording, or the control unit is not online", () => {
    const predicate = Expr.compile(
      BooleanTree.or([Expr.ref("suitest_camera_recording"), BooleanTree.not(Expr.ref("suitest_control_unit_online"))]),
    );
