...
describe("workflow/condition", () => {
  it("evaluates fact leaves against the env lookup, exactly like a tripwire predicate", async () => {
    ...
    expect(await Condition.evaluate(Predicates.ref("connected"))(env)()).toStrictEqual(E.right(true));
    expect(await Condition.evaluate(Predicates.ref("recording"))(env)()).toStrictEqual(E.right(false));
  });
  ...
  it("mixes fact and probe leaves in the same tree", async () => {
    ...
    const condition = BooleanTree.and([BooleanTree.not(Predicates.ref("connected")), Condition.probe("screenOn")]);
  ...
  it("short-circuits `or`: a probe is not run once a fact has decided", async () => {
    ...
    await Condition.evaluate(BooleanTree.or([Predicates.ref("connected"), Condition.probe("screenOn")]))(env)();
    expect(calls).toBe(0);
  });
  ...
  it("short-circuits `and` on the first false", async () => {
    ...
    await Condition.evaluate(BooleanTree.and([Predicates.ref("connected"), Condition.probe("screenOn")]))(env)();
    expect(calls).toBe(0);
  });
  ...
  it("fails when the env carries no lookup, rather than reading every fact as false", async () => {
    expect(E.isLeft(await Condition.evaluate(Predicates.ref("connected"))(envWith({}))())).toBe(true);
  });
});
