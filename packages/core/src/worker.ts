...
export const create = (
  ...
  const tick = async (): Promise<void> => {
    ...
    if (delay === null) {
      workerLogger.info(`${formattedJobLabel} exhausted - stopping`)();
      return;
    }

    status = { iteration: status.iteration + 1, previousDelay: delay };

    workerLogger.debug(`${formattedJobLabel} - Tick: ${status.iteration} - Next delay: ${Logger.formatMs(delay)}`)();
  ...
  return {
    start: pipe(
      TE.fromIO(workerLogger.info(`Starting ${formattedJobLabel}`)),
      TE.flatMap(() => TE.tryCatch(() => tick(), Errors.fromUnknown("StartError"))),
    ),
    stop: pipe(
      workerLogger.info(`Stopping ${formattedJobLabel}`),
